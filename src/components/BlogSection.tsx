...
import { Calendar, Eye, Heart, MessageCircle, Search, Tag, User } from 'lucide-react';
...
const BlogSection = () => {
  ...
  const dummyBlogs: BlogData[] = [{
    ...
    images: ['/lovable-uploads/monastery-1.jpg', '/lovable-uploads/monastery-2.jpg'],
    ...
    images: ['/lovable-uploads/monastery-2.jpg', '/lovable-uploads/monastery-3.jpg', '/lovable-uploads/monastery-1.jpg'],
    ...
    images: ['/lovable-uploads/monastery-3.jpg', '/lovable-uploads/monastery-1.jpg', '/lovable-uploads/monastery-2.jpg'],
    ...
    featured_image: '/lovable-uploads/monastery-1.jpg',
    images: ['/lovable-uploads/monastery-1.jpg', '/lovable-uploads/monastery-3.jpg'],
    ...
    featured_image: '/lovable-uploads/monastery-2.jpg',
    images: ['/lovable-uploads/monastery-2.jpg', '/lovable-uploads/monastery-1.jpg', '/lovable-uploads/monastery-3.jpg'],
  ...
  const filteredBlogs = dummyBlogs.filter(blog => {
    const matchesSearch = searchTerm === '' || blog.title.toLowerCase().includes(searchTerm.toLowerCase()) || blog.content.toLowerCase().includes(searchTerm.toLowerCase()) || blog.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesTags = selectedTags.length === 0 || selectedTags.some(selectedTag => blog.tags.includes(selectedTag));
    return matchesSearch && matchesTags;
  });
  const allTags = Array.from(new Set(dummyBlogs.flatMap(blog => blog.tags)));
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredBlogs.map(blog => <Card key={blog.id} className="group hover:shadow-lg transition-all duration-300 bg-card/80 backdrop-blur-sm">
            <div className="aspect-video overflow-hidden rounded-t-lg">
              <OptimizedImage src={blog.featured_image} alt={blog.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" width={400} height={225} />
            </div>
            <CardHeader>
              ...
              <div className="flex items-center justify-between pt-2">
                <div className="flex items-center space-x-4 text-sm text-foreground/70">
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    <span>{Math.floor(blog.views_count * 0.1)}</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <MessageCircle className="w-4 h-4" />
                    <span>{Math.floor(blog.views_count * 0.05)}</span>
                  </div>
                </div>
                <Dialog>
                  ...
                  <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto bg-background">
                    ...
                    <div className="py-6 space-y-8">
                      <div className="aspect-video max-w-2xl mx-auto overflow-hidden rounded-lg shadow-md">
                        <OptimizedImage src={blog.featured_image} alt={blog.title} className="w-full h-full object-cover" loading="eager" width={600} height={337} />
                      </div>
                      ...
                      {blog.images.length > 1 && <div className="space-y-6 pt-8 border-t border-border">
                          <h4 className="text-lg font-semibold text-foreground">Additional Images</h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {blog.images.slice(1).map((image, index) => <div key={index} className="space-y-3">
                                <div className="aspect-video overflow-hidden rounded-lg shadow-md">
                                  <OptimizedImage src={image} alt={`Blog image ${index + 2}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" loading="lazy" width={400} height={225} />
                                </div>
                              </div>)}
                          </div>
                        </div>}
